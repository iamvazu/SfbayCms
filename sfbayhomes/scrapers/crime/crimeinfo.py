#-------------------------------------------------------------------------------
# Name:        crime info
# Purpose:  Grabs all crime related data from city-data
#
# Author:      Lord Azu
#
# Created:     18/01/2017
...
def grab_crime_data():
    ...
    s = requests.session()
    state_links = ['http://www.city-data.com/crime/crime-California.html', 'http://www.city-data.com/crime/crime-California2.html']#get_state_links(s)
    fileout = create_file()
    process_states(s, state_links, fileout)
    s.close()
    print ('finished crime info')
